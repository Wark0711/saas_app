    ...
    <>
      <section className="min-h-screen bg-[radial-gradient(hsl(0,72%,65%,40%),hsl(24,62%,73%,40%),hsl(var(--background))_60%)] flex items-center justify-center text-center text-balance flex-col gap-8 px-4">
        ...
        <p className="text-lg lg:text-3xl max-w-screen-xl">
          Optimize your product procing across countries to maximize sales.
      ...
      <section className="bg-primary text-primary-foreground">
        <div className="container py-16 flex flex-col gap-16 px-8 md:px-16">
          <h2 className="text-3xl text-center text-balance semibold">Trusted by top modern companies</h2>
          <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-16">
            <Link href={'https://neon.tech/h'}>
              <NeonIcon />
            </Link>
  ...
  return (
    <Card className={cn('relative shadow-none rounded-3xl overflow-hidden', isMostPopular ? 'border-accent border-2' : 'border-none')}>
      ...
      <CardFooter className="flex flex-col gap-4 items-start">
        ...
        {canCustomizeBanner && <Feature>Advanced customization</Feature>}
