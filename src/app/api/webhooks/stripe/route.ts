...
import { updateUserSubscription } from "@/server/db/subscritpion";
...
export async function POST(request: NextRequest) {
    const event = await stripe.webhooks.constructEvent(await request.text(), request.headers.get('stripe-signature') as string, env.STRIPE_WEBHOOK_SECRET)
    switch (event.type) {
