...
export async function getProductCountryGroups({ productId, userId }: { productId: string, userId: string }) {
...
export async function updateCountryDiscounts(
    ...
    if (insertGroup.length > 0) {
        statements.push(db.insert(CountryGroupDiscountTable).values(insertGroup).onConflictDoUpdate({
            target: [
